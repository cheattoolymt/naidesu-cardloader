 ...
 *
 * ● 余白を削って“紙を使い切る”設計 (v4) ------------------------------
 *   旧レイアウトは端余白 QUIET=150px(12.7mm) と広めのフッタ/隙間で、
 *   データグリッドは A4 の 59.4% しか使っていなかった。家庭用プリンタの
 *   実際の印刷可能領域(概ね上下左右 5mm 前後)には十分な余裕があるので、
 *   端余白を QUIET=90px(7.6mm) まで削り、ファインダ/隙間/フッタも切り詰めた。
 *   これでデータグリッドは 2020 x 2978 px (= A4 の 69.1%) まで拡大。
 *   同じセル物理サイズでも約 1.15 倍のバイトが載る(=無駄の除去)。
 *
 * ● 密度モード (8種) ------------------------------------------------
 *   セルは「300dpi でも余裕で読める」よう最小でも ≈0.81mm を確保する。
 *   余白削減でグリッドが広がったぶん、同じセルサイズでも容量が増える。
 *   "1kb":  96 x 143 セル ->  1692 byte/枚 (cell ≈ 1.78mm)
 *   "2kb": 112 x 167 セル ->  2310 byte/枚 (cell ≈ 1.52mm)
 *   "3kb": 130 x 194 セル ->  3120 byte/枚 (cell ≈ 1.31mm)
 *   "4kb": 149 x 222 セル ->  4116 byte/枚 (cell ≈ 1.14mm)
 *   "5kb": 170 x 254 セル ->  5376 byte/枚 (cell ≈ 1.00mm) ← 旧0.93→1.00mmに余裕UP
 *   "6kb": 183 x 270 セル ->  6153 byte/枚 (cell ≈ 0.94mm) ← 旧5kbと同じセルで6KB超
 *   "7kb": 198 x 292 セル ->  7202 byte/枚 (cell ≈ 0.86mm)
 *   "8kb": 211 x 311 セル ->  8176 byte/枚 (cell ≈ 0.81mm) ← 最高密度・圧縮併用で実質10KB超
 *
 *   グリッドはほぼ正方形セル(アスペクト比 ≈ 1.00)。300dpi でのセル物理サイズは
 *     1kb: ≈ 1.78mm   4kb: ≈ 1.14mm   7kb: ≈ 0.86mm
 *     2kb: ≈ 1.52mm   5kb: ≈ 1.00mm   8kb: ≈ 0.81mm
 *     3kb: ≈ 1.31mm   6kb: ≈ 0.94mm
 *
 * ● 誤り訂正 (ECC) — 段階選択 --------------------------------------
...
(function (global) {
  ...
  // v4: 端余白を切り詰めて A4 を使い切る(旧 QUIET=150/FINDER=120/GAP=40/FOOTER=110)。
  //   QUIET  90px = 7.6mm  … 家庭用プリンタの印刷可能領域に十分収まる安全余白
  //   FINDER 110px = 9.3mm … 位置検出に必要な四隅マーカー(縮めても検出は安定)
  //   GAP     30px = 2.5mm … ファインダとデータグリッドの隙間(混同防止に最小限)
  //   FOOTER  70px = 5.9mm … 人間可読フッタ用(28px フォント1行に必要な最小限)
  const QUIET = 90; // px
  const FINDER = 110; // px 正方形マーカー
  const GAP = 30; // px
  const FOOTER = 70; // px 下端の人間可読テキスト用に確保する余白
  ...
  const GRID_X = QUIET + FINDER + GAP; // 230
  const GRID_Y = QUIET + FINDER + GAP; // 230 (上端)
  const GRID_W = PAGE_W - 2 * GRID_X;  // 2020 (旧 1860 → 余白削減で拡大)
  const GRID_H = PAGE_H - GRID_Y - GRID_X - FOOTER; // 2978 (旧 2778 → 拡大)
  ...
  // 共通長方形(2020 x 2978)を細分する cols x rows。payload はページ全面を活用。
  // PAYLOAD_BYTES は「グロス容量」(グリッドが物理的に持てるバイト数)。
  // ECC を使う場合の正味容量は netPayload(mode, eccLevel) で求める。
  // cols/rows は 2020:2978 ≈ 0.678 に合わせてあり、セルはほぼ正方形(ar≈1.0)。
  //   1kb:  96x143 -> 1692B (cell ≈ 1.78mm)   5kb: 170x254 -> 5376B (cell ≈ 1.00mm)
  //   2kb: 112x167 -> 2310B (cell ≈ 1.52mm)   6kb: 183x270 -> 6153B (cell ≈ 0.94mm)
  //   3kb: 130x194 -> 3120B (cell ≈ 1.31mm)   7kb: 198x292 -> 7202B (cell ≈ 0.86mm)
  //   4kb: 149x222 -> 4116B (cell ≈ 1.14mm)   8kb: 211x311 -> 8176B (cell ≈ 0.81mm)
  const PROFILES = {
    ...
    '6kb': makeProfile('6kb', 183, 270),
    '7kb': makeProfile('7kb', 198, 292),
    '8kb': makeProfile('8kb', 211, 311),
  };
